...
`frontend/src/components/FilterBar.tsx:124-126` only offers "All sources / Yad2 / Madlan".
Once FB ingestion is live, add a third `<option value="facebook">Facebook</option>`.
Confirm the backend list endpoint (`/api/properties`) already filters by `source='facebook'` and the
rows show up in the dashboard. Small but user-visible — do after A2 ships and real FB data lands.

### Apply `normalize_city()` helper across Yad2/Madlan ingest + filters
The A2 brief introduces `pdis.utils.city.normalize_city()` and uses it on the FB ingest path only
(to avoid breaking 1,952 existing rows). Once canonical is proven stable, extend:
- `pdis/scraper.py:182` — apply to Yad2 `item.get("city")` on ingest
- `pdis/scraper_madlan.py:250` — apply to Madlan `addr.get("city")`
- Any route that filters `properties.address_city` by string equality (search filter, neighborhood stats)
Goal: one canonical value per city across all sources, so cross-source dedup and neighborhood aggregates stop
depending on exact Yad2/Madlan spelling.
