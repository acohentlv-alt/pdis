# PDIS — Task List
*April 25, 2026 (carried forward from Apr 23; Apr 25 session added FB restart planning)*

---

## 📋 PLANNING IN PROGRESS — FB scraper restart (Brief v3 pending)

**Session Apr 25 context:** Investigated ho-me.app + 4kirot architecture (both use Apify, same actor as PDIS, no secret tech). Ran `/plan` → Path A chosen (self-host `curl_cffi` against `mbasic.facebook.com` on Oracle VM, using Alan's personal FB cookies, $0 marginal scraping cost). Brief v1 and v2 both returned REVISE from reviewer — brief v2's issues were mostly because it was drafted against a stale CLAUDE.md in worktree `youthful-sammet`, not current `~/pdis/` state. Many proposed removals already happened in PR #3.

**Blocked on 3 Alan answers** before drafting Brief v3:
1. **14 or 50 groups for MVP?** At 14, cost is ~$4/mo worst case (trivial). At 50, it's tight vs $15 ceiling.
2. **Cookie refresh strategy:** manual Chrome export → scp every 2–4 weeks (recommended), browser-extension one-click, or Playwright auto-login helper on VM.
3. **Iteration path:** patch Brief v2 inline + `/review` again, or patch + skip to `/exec` with reviewer notes?

**Current FB state:** Apify credit exhausted (402 Payment Required). FB ingest = $0/day, not the "$5/day bleed" mentioned in earlier handoffs. No timeline pressure for restart.

**Relevant artifacts:**
- Session transcript (including Brief v2 and reviewer's 10 REVISE findings) — not saved to file; pull from conversation when resuming
- Brief v2 removal list was based on stale worktree info; v3 should remove only what's actually still present: `vm-scraper/apify_to_pdis.py`, `APIFY_TOKEN`/`PROXY_URL` env vars, Apify refs in CLAUDE.md

**Alternative next-steps if Path A stalls:**
- Top up Apify credits and continue on current pipeline (known-working, budget depends on volume)
- Stay on 14 groups indefinitely — less ambitious but lower risk
